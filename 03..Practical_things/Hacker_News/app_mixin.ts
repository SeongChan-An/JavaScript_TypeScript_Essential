...
// type 형태가 익숙해지기 위해 이 코드만 주석으로 남겨놓는다.
// 실제 업무에서는 남겨놓지 않는다.
// type NewsFeed = News & {
//   comments_count: number;
//   points: number;
//   read?: boolean; // optional 있을 수도 있고 없을수도 있을 때 ? 사용
// };
...
class NewsFeedView {
  constructor() {
    ...
    const newsList = [];
    const maxPage = Object.keys(newsFeed).length / MAX_ITEM;
    let template = `
  ...
  `;
    ...
    for (
      let i = (store.currentPage - 1) * MAX_ITEM;
      i < store.currentPage * MAX_ITEM;
      i++
    ) {
      newsList.push(`
     <div class="p-6 ${newsFeed[i].read ? "bg-gray-600" : "bg-white"} 
...
function updateView(html: string): void {
  if (container) {
    ...
    console.error("최상위 컨테이너가 없어 UI 작업 진행을 할 수 없습니다.");
  }
}
...
function newsFeed(): void {
  ...
  for (
    ...
    newsList.push(`
     <div class="p-6 ${newsFeed[i].read ? "bg-cyan-600" : "bg-white"} 
     mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
        <div class="flex">
          <div class="flex-auto">
            <a href="#/show/${newsFeed[i].id}">${newsFeed[i].title}</a>  
...
function newsDetail(): void {
  ...
  const newsContent = api.getData(CONTENT_URL.replace("@id", id));
